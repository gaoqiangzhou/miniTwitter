...
export function PostContextProvider({ children }) {
  ...
  //use this to sort (likes-dislike) and make it decrease
  useEffect(() => {
  ...
  const read = (userId, postId) => {
    ...
    axios.post(READ_API(postId), { userId: userId }).then((res) => {
      if (reads.reduce((acc, cur) => acc && cur != userId, true)) {
        setPosts(
          posts.map((ea) =>
            ea._id === post._id
              ? {
  ...
  const like = (userId, postId) => {
    ...
    //if already liked?
    if (likes.reduce((acc, cur) => acc || userId === cur.user, false)) {
      //cancel like
      axios
        .put(LIKAPI(postId), { userId: userId })
        .then((res) => {
          setPosts(
            posts.map((ea) =>
              ea._id === post._id
      ...
      //if disliked?
      if (dislikes.reduce((acc, cur) => acc || userId === cur.user, false)) {
        //cancel dislike
        axios
          .put(DISLIKAPI(postId), { userId: userId })
          .then((res) => {
            setPosts((prev) =>
              prev.map((ea) =>
                ea._id === post._id
      ...
      //add like
      axios
        .post(LIKAPI(postId), { userId: userId })
        .then((res) => {
          setPosts((prev) =>
            prev.map((ea) =>
              ea._id === post._id
  ...
  const dislike = (userId, postId) => {
    ...
    if (dislikes.reduce((acc, cur) => acc || userId === cur.user, false)) {
      ...
      //add dislike
      axios
        .post(DISLIKAPI(postId), { userId: userId })
        .then((res) => {
          setPosts((prev) =>
            prev.map((ea) =>
              ea._id === postId
          ...
          prev?.map((ea) =>
            ea._id === postId
              ? {
                  ...
                  complaints: [
                    ...complaints,
                    { by: userId, reason: reason, _id: res.data.complain._id },
                  ],
                }
              : ea
          )
  ...
  useEffect(() => {
    axios.get(postAPI).then((res) => setPosts(res.data));
  }, []);
